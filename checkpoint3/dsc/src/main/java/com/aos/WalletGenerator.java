...
public class WalletGenerator {
    ...
    public double getBalance() {
        ...
        try {
            ...
            double response = stub.getBalance(Base58.encode(publickey));
            System.out.println("Your Wallet Balance is: " + response + "for your Address:" + Base58.encode(publickey));
            return response;
        } catch (Exception e) {
    ...
    public String send(double amount, String recipientAddress) throws Exception {
        ...
        Transaction t = new Transaction(publickey, Base58.decode(recipientAddress), generateTransactionID(), amount,
    ...
    public String send1(double amount, String recipientAddress) throws Exception {
        //double balance = getBalance();
        Transaction t = new Transaction(publickey, Base58.decode(recipientAddress), generateTransactionID(), amount,
                System.currentTimeMillis());
        String transactionId = Base58.encode(t.getTransitionId());
    ...
    public String trxstatus(String transactionId) {
        ...
        try {
            registry = LocateRegistry.getRegistry(IP, Integer.valueOf(port));
            PoolInf stub = (PoolInf) registry.lookup("PoolService");
             response = stub.Status(transactionId);
