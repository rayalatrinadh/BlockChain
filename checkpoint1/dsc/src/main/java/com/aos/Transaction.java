...
public class Transaction implements Serializable{
    ...
    public Transaction(byte[] senderAddress, byte[] recipientAddress, byte[] transactionId, double amount, long timestamp) {
...
/*@Deprecated
    private byte[] genHashTransactionID() {
        try {
            ...
            byte[] id = digest.digest((Arrays.toString(senderAddress) + Arrays.toString(recipientAddress) + amount + timestamp).getBytes());
   ...
   private void generateSignature() {
        try {
            ...
            // KeyFactory keyFactory = KeyFactory.getInstance("RSA"); // Adjust the algorithm as needed
