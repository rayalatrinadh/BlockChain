...
public class Metronome extends UnicastRemoteObject implements MetronomeInf {
    ...
    public void getprevioushash() {
        ...
        try {
            ...
            System.out.println("previous hash: " + Base58.encode(prev.calhash()) + " blockID" + prev.getblockID());
        } catch (Exception e) {
            // TODO Auto-generated catch block
                    ...
                    try {
                        if (winners.size() > 0) {
                            for (String winner : winners) {
                                ...
                                difficulty = (difficulty + 5 > 28 ? difficulty + 5 : 30);
                            }
                        } else {
                            genBlock();
                            difficulty = (difficulty - 5 > 0 ? difficulty - 5 : 1);
                        }
                    } catch (Exception e) {
                        // TODO Auto-generated catch block
                        e.printStackTrace();
    ...
    public boolean winner(byte[] pkBytes, byte[] fpBytes, int nonce, int threadId) {
        ...
        if (compareByteArrays(hash, toCompare, difficulty)) {
            ...
            System.err.println("Winner " + true +" nonce "+nonce+" difficulty"+difficulty);
            return true;
        }
    ...
    private void rewards(String publickey) throws Exception {
        if (rewards > 1) {
            ...
            for (String w : winners) {
                Transaction t = new Transaction(Base58.decode(w), Base58.decode(publickey), generateTransactionID(),
